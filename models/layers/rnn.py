...
class DWSConvLSTM2d(nn.Module):
    ...
    def __init__(self,
                 dim: int,
                 dws_conv: bool = True,  # RVT uses False
        ...
        # `self.conv3x3_dws` is actually just Identity mapping in RVT
    ...
    def forward(self, x: th.Tensor, h_and_c_previous: Optional[Tuple[th.Tensor, th.Tensor]] = None) \
        ...
        """
        :param x: (N C H W), new features extracted at current time step
        :param h_and_c_previous: ((N C H W), (N C H W)), prev RNN states
        ...
        xh = th.cat((x, h_tm1), dim=1)  # (N 2C H W)
