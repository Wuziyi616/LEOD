...
class RNNDetector(BaseDetector):
    """RNN-based backbone with MaxViT blocks."""
    ...
    def __init__(self, mdl_config: DictConfig):
        ...
        # TODO: doesn't work if we compile the forward function here
        # TODO: instead, we should compile the entire model outside here
    ...
    def forward(self, x: th.Tensor, prev_states: Optional[LstmStates] = None, token_mask: Optional[th.Tensor] = None) \
        ...
        """Forward pass with potential history information recurrency.

        Returns:
            output: Dict{stage_id: feats, [B, C, h, w]}, output of each stage
...
class RNNDetectorStage(nn.Module):
    ...
    def __init__(self,
        ...
        ###### Mask Token ################
        # learnable [MASK] token put to padded pixels
    ...
    def forward(self, x: th.Tensor,
        ...
        """Forward pass as a single-stage RNN-based MaxViT layer.

        Returns:
            x: [B, C, H, W], current feature (after applying RNN)
            h_and_c_previous: Tuple([B, C, H, W], [B, C, H, W]), updated RNN states
        """
        ...
        for blk in self.att_blocks:
            x = blk(x)
        x = nhwC_2_nChw(x).contiguous()  # N H W C -> N C H W
