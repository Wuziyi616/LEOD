...
class InputPadderFromShape:
    ...
    def __init__(self, desired_hw: Tuple[int, int], mode: str = 'constant',
                 value: int = 0, type: str = 'corner'):
        """
    ...
    @staticmethod
    def _pad_tensor_impl(input_tensor: th.Tensor, desired_hw: Tuple[int, int],
                         mode: str, value: Any) -> Tuple[th.Tensor, List[int]]:
        ...
        return F.pad(input_tensor, pad=pad, mode=mode, value=value if
                     mode == 'constant' else None), pad

    def pad_tensor_ev_repr(self, ev_repr: th.Tensor) -> th.Tensor:
        padded_ev_repr, pad = self._pad_tensor_impl(
            input_tensor=ev_repr, desired_hw=self.desired_hw,
    ...
    def pad_token_mask(self, token_mask: th.Tensor):
        ...
        padded_token_mask, pad = self._pad_tensor_impl(
            input_tensor=token_mask, desired_hw=desired_hw,
