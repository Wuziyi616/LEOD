...
YOLOX_PRED_PROCESSED = List[Optional[Union[th.Tensor, np.ndarray, ObjectLabels]]]
...
def to_prophesee(loaded_label_list: LOADED_LABELS, yolox_pred_list: YOLOX_PRED_PROCESSED) -> \
    ...
    # loaded_label_list: List[ObjectLabels, each (N, 7)], [t, (x,y,w,h), cls_idx, cls_conf]
    # yolox_pred_list: List[(N_i, 7)], [(x1,y1,x2,y2), obj_conf, cls_conf, cls_idx]
    #                  can also be List[ObjectLabels], need to convert to np
    yolox_pred_list = [
        pred.get_labels_as_tensors('prophesee')
        if isinstance(pred, ObjectLabels) else pred for pred in yolox_pred_list
    ]
    # process and make both results [t, (x,y,w,h), cls_idx, cls_conf]
    ...
    for loaded_labels, yolox_preds in zip(loaded_label_list, yolox_pred_list):
        ...
        for name in BBOX_DTYPE.names:
            ...
            if name == 't':
                ...
                assert time.size == 1, 'All labels should come from the same frame'
                time = time.item()
        ...
        num_pred = 0 if yolox_preds is None else yolox_preds.shape[0]  # [N2, 7]
