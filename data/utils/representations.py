...
class StackedHistogram(RepresentationBase):
    ...
    def construct(self, x: th.Tensor, y: th.Tensor, pol: th.Tensor, time: th.Tensor) -> th.Tensor:
        ...
        ch, bn, ht, wd = self.channels, self.bins, self.height, self.width
        ...
        t_norm = t_norm * bn  # 0<=t_norm<=bn
        t_idx = t_norm.floor()  # which bin it belongs to
        t_idx = th.clamp(t_idx, max=bn - 1)  # 0<=t_idx<=bn-1
        ...
        representation = th.clamp(representation, min=0, max=self.count_cutoff)  # remove hotpixels?
        if not self.fastmode:
            representation = representation.to(th.uint8)

        return self.merge_channel_and_bins(representation)  # [2*B, H, W]
